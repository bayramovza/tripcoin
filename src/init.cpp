...
void Shutdown()
{
    ...
    GenerateTripcoins(false, NULL, 0);
#endif
    StopNode();
...
std::string HelpMessage(HelpMessageMode mode)
{
    ...
    strUsage += HelpMessageOpt("-conf=<file>", strprintf(_("Specify configuration file (default: %s)"), "tripcoin.conf"));
    if (mode == HMM_TRIPCOIND)
    {
#if !defined(WIN32)
...
#ifndef WIN32
    strUsage += HelpMessageOpt("-pid=<file>", strprintf(_("Specify pid file (default: %s)"), "tripcoind.pid"));
#endif
    ...
    if (showDebug)
        strUsage += HelpMessageOpt("-mintxfee=<amt>", strprintf("Fees (in TPC/Kb) smaller than this are considered zero fee for transaction creation (default: %s)",
    ...
    strUsage += HelpMessageGroup(_("RPC SSL options: (see the Tripcoin Wiki for SSL setup instructions)"));
...
{
    return FormatParagraph(strprintf(_("Copyright (C) 2009-%i The Tripcoin Core Developers"), COPYRIGHT_YEAR)) + "\n" +
           "\n" +
...
/** Sanity checks
 *  Ensure that Tripcoin is running in a usable environment with all
...
bool InitSanityCheck(void)
{
    if(!ECC_InitSanityCheck()) {
        InitError("OpenSSL appears to lack support for elliptic curve cryptography. For more "
                  "information, visit https://en.tripcoin.it/wiki/OpenSSL_and_EC_Libraries");
        return false;
    }
...
#define PROCESS_DEP_ENABLE 0x00000001
    ...
    // Sanity check
    if (!InitSanityCheck())
        return InitError(_("Initialization sanity check failed. Tripcoin Core is shutting down."));
...
#endif
    // Make sure only a single Tripcoin process is using the data directory.
    ...
    try {
        ...
        if (!lock.try_lock())
            return InitError(strprintf(_("Cannot obtain a lock on data directory %s. Tripcoin Core is probably already running."), strDataDir));
    } catch(const boost::interprocess::interprocess_exception& e) {
        return InitError(strprintf(_("Cannot obtain a lock on data directory %s. Tripcoin Core is probably already running.") + " %s.", strDataDir, e.what()));
    }
    ...
    if (fDisableWallet) {
        ...
        if (nLoadWalletRet != DB_LOAD_OK)
        {
            ...
            else if (nLoadWalletRet == DB_NONCRITICAL_ERROR)
            {
                ...
                strErrors << _("Error loading wallet.dat: Wallet requires newer version of Tripcoin Core") << "\n";
            else if (nLoadWalletRet == DB_NEED_REWRITE)
            {
                strErrors << _("Wallet needed to be rewritten: restart Tripcoin Core to complete") << "\n";
    ...
    if (pwalletMain)
        GenerateTripcoins(GetBoolArg("-gen", false), pwalletMain, GetArg("-genproclimit", 1));
#endif
